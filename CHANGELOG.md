...
- 将服装款式扩充至 137 款，保留原有 31 款的规范 ID 和工作流选项，并将普通毛衣 `sweater_casual` 与露背毛衣 `knit_sweater` 分开建模。
- 新增 11 条服装状态，总计 23 条；服装修饰、内衣缺席声明和叠穿状态分别按其语义判定。
- 增加 7 项配饰、2 项内衣和 1 项身体微瑕；只有具备相应形制且在穿的服装实体可承载衣物状态。
- 加强 `target_id` 绑定、款式形制能力检查和冲突诊断；目标不存在或不兼容时不自动改绑其他衣物。
- 建立服装迁移台账、分阶段双版本差异审计与 10,000 种子黄金哈希门禁。第 3 步本地全量 413 项测试通过，未解释差异为 0。

同一 `prompt_seed` 在 rc8 与 rc9 之间可能产生不同提示词；同一 rc9 版本及相同输入保持确定性。
